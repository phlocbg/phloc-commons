...
public abstract class RandomAccessLongList extends AbstractLongCollection implements LongList
{
  ...
  public abstract long get (int nIndex);
   ...
   *         since this method is not supported
   */
  public long set (final int index, final long aElement)
  {
    throw new UnsupportedOperationException ();
  }
   ...
   *         since this method is not supported
   */
  public void add (final int index, final long aElement)
  {
    throw new UnsupportedOperationException ();
  }
  ...
  public boolean addAll (final int nIndex, @Nonnull final LongCollection aCollection)
  {
    int index = nIndex;
    boolean bModified = false;
    for (final LongIterator iter = aCollection.iterator (); iter.hasNext ();)
    {
      add (index++, iter.next ());
      bModified = true;
    }
  ...
  {
    for (final LongListIterator iter = listIterator (size ()); iter.hasPrevious ();)
  ...
  {
    return new RandomAccessLongListIterator (this, nIndex);
  }

  public LongList subList (final int nFromIndex, final int nToIndex)
  {
    return new RandomAccessLongSubList (this, nFromIndex, nToIndex);
  }
  ...
  {
    private RandomAccessLongList m_aSource;
    private int m_nExpectedModCount = -1;

    ComodChecker (@Nonnull final RandomAccessLongList source)
    {
      m_aSource = source;
      resyncModCount ();
    }
  ...
  protected static class RandomAccessLongListIterator extends ComodChecker implements LongListIterator
  {
    ...
    public long next ()
    {
      ...
      final long val = getList ().get (m_nNextIndex);
    ...
    public long previous ()
    {
      ...
      if (!hasPrevious ())
        throw new NoSuchElementException ();
      final long val = getList ().get (m_nNextIndex - 1);
  ...
  protected static class RandomAccessLongSubList extends RandomAccessLongList
  {
    ...
    private RandomAccessLongList m_aList;
    private ComodChecker m_aComod;

    RandomAccessLongSubList (@Nonnull final RandomAccessLongList list, @Nonnegative final int fromIndex, @Nonnegative  final int toIndex)
    {
    ...
    public long removeElementAt (final int index)
    {
      checkRange (index);
      m_aComod.assertNotComodified ();
      final long val = m_aList.removeElementAt (toUnderlyingIndex (index));
      m_nLimit--;
      m_aComod.resyncModCount ();
      incrModCount ();
      return val;
    ...
    public long set (final int index, final long element)
    {
      checkRange (index);
      m_aComod.assertNotComodified ();
      final long val = m_aList.set (toUnderlyingIndex (index), element);
      incrModCount ();
      m_aComod.resyncModCount ();
      return val;
    }
