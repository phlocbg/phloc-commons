...
public class ArrayBooleanList extends RandomAccessBooleanList implements Serializable
{
  private transient boolean [] m_aData;
  private int m_nSize;
   ...
   */
  public ArrayBooleanList (final int nInitialCapacity)
  {
    if (nInitialCapacity < 0)
      throw new IllegalArgumentException ("capacity " + nInitialCapacity);
    m_aData = new boolean [nInitialCapacity];
    m_nSize = 0;
  }
   ...
   * @param that
   *        the non-<code>null</code> collection of <code>boolean</code>s to add
   * @throws NullPointerException
   *         if <i>that</i> is <code>null</code>
   */
  public ArrayBooleanList (@Nonnull final BooleanCollection that)
  {
    this (that.size ());
    addAll (that);
  }
   ...
   */
  public ArrayBooleanList (@Nonnull final boolean [] array)
  {
    this (array.length);
  ...
  public boolean removeElementAt (final int index)
  {
    _checkRange (index);
    incrModCount ();
    final boolean oldval = m_aData[index];
  ...
  @Override
  public boolean addAll (@Nonnull final BooleanCollection collection)
  {
  ...
  public void ensureCapacity (final int nMinCap)
  {
    ...
    if (nMinCap > m_aData.length)
    {
      ...
      final boolean [] olddata = m_aData;
      m_aData = new boolean [newcap < nMinCap ? nMinCap : newcap];
  ...
  private void writeObject (@Nonnull final ObjectOutputStream out) throws IOException
  {
    ...
    for (int i = 0; i < m_nSize; i++)
      out.writeBoolean (m_aData[i]);
  }

  private void readObject (@Nonnull final ObjectInputStream in) throws IOException, ClassNotFoundException
  {
    in.defaultReadObject ();
    m_aData = new boolean [in.readInt ()];
    for (int i = 0; i < m_nSize; i++)
      m_aData[i] = in.readBoolean ();
  }
