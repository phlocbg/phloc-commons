...
public class ArrayFloatList extends RandomAccessFloatList implements Serializable
{
  private transient float [] m_aData;
  private int m_nSize;
   ...
   */
  public ArrayFloatList (final int nInitialCapacity)
  {
    if (nInitialCapacity < 0)
      throw new IllegalArgumentException ("capacity " + nInitialCapacity);
    m_aData = new float [nInitialCapacity];
    m_nSize = 0;
  }
   ...
   *
   * @see ArrayFloatList#addAll(FloatCollection)
   * @param that
   *        the non-<code>null</code> collection of <code>float</code>s to add
   * @throws NullPointerException
   *         if <i>that</i> is <code>null</code>
   */
  public ArrayFloatList (@Nonnull final FloatCollection that)
  {
    this (that.size ());
    addAll (that);
  }
   ...
   */
  public ArrayFloatList (@Nonnull final float [] array)
  {
    this (array.length);
  ...
  public float removeElementAt (final int index)
  {
    _checkRange (index);
    incrModCount ();
    final float oldval = m_aData[index];
  ...
  @Override
  public boolean addAll (@Nonnull final FloatCollection collection)
  {
  ...
  public void ensureCapacity (final int nMinCap)
  {
    ...
    if (nMinCap > m_aData.length)
    {
      ...
      final float [] olddata = m_aData;
      m_aData = new float [newcap < nMinCap ? nMinCap : newcap];
  ...
  private void writeObject (@Nonnull final ObjectOutputStream out) throws IOException
  {
    ...
    for (int i = 0; i < m_nSize; i++)
      out.writeFloat (m_aData[i]);
  }

  private void readObject (@Nonnull final ObjectInputStream in) throws IOException, ClassNotFoundException
  {
    in.defaultReadObject ();
    m_aData = new float [in.readInt ()];
    for (int i = 0; i < m_nSize; i++)
      m_aData[i] = in.readFloat ();
  }
