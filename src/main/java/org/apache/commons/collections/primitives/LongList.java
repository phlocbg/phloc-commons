...
public interface LongList extends LongCollection
{
   ...
   */
  void add (int nIndex, long aElement);
   ...
   *        collection
   * @param aCollection
   *        the {@link LongCollection ByteCollection} of elements to add
   * @return <code>true</code> if I changed as a result of this call
   * @throws UnsupportedOperationException
   *         when this operation is not supported
   * @throws IndexOutOfBoundsException
   *         if the specified index is out of range
   */
  boolean addAll (int index, @Nonnull LongCollection aCollection);

  /**
   * Returns <code>true</code> if <i>that</i> is an <code>LongList</code>
   * that contains the same elements in the same order as me. In other words,
   * returns <code>true</code> if <i>that</i> is a <code>LongList</code> that
   * has the same {@link #size() size} as me, and for which the elements
   * returned by its {@link LongList#iterator iterator} are equal (
   ...
   * the <code>LongList</code> interface.)
   *
   * @param that
   *        the object to compare to me
   * @return <code>true</code> if <i>that</i> is an <code>LongList</code>
  ...
  /**
   * The hash code of an {@link LongList} is defined to be the result of the
   ...
   *         <code>-1</code> if no such matching element can be found
   */
  int lastIndexOf (long aElement);

  /**
   * @return a {@link LongListIterator bidirectional iterator} over all my
   ...
   */
  @Nonnull
  LongListIterator listIterator (int nIndex);
   ...
   *         if the specified index is out of range
   */
  long removeElementAt (int nIndex);
   ...
   *         if the specified index is out of range
   */
  long set (int nIndex, long aElement);
   ...
   * <code>LongList</code> is backed by me, so that any changes in the
   ...
   */
  @Nonnull
  LongList subList (int nFromIndex, int nToIndex);
}
