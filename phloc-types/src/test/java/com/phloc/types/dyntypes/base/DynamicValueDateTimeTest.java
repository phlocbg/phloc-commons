...
/**
 * Test class for class {@link DynamicValueDateTime}.
 * 
 * @author Philip Helger
 */
public final class DynamicValueDateTimeTest extends AbstractPhlocTestCase
{
  private static String _cutTimeZone (final String s)
  {
  ...
  public void testAll ()
  {
    final DynamicValueDateTime aDTD = new DynamicValueDateTime ();
    assertNull (aDTD.getValue ());
    DateTime aNow = PDTFactory.getCurrentDateTime ();
    aDTD.setValue (aNow);
    assertEquals (aNow, aDTD.getValue ());
    aNow = PDTFactory.createDateTime (2010, 5, 12);
    aDTD.setValue (aNow);
    assertEquals (aNow, aDTD.getValue ());

    // Set as java.util.Date
    assertTrue (aDTD.setValue (new Date (1270936800000L)).isChanged ());
    assertEquals (PDTFactory.createDateTime (2010, DateTimeConstants.APRIL, 11), aDTD.getValue ());
    // Set as GregorianCalendar
    assertTrue (aDTD.setValue (new GregorianCalendar (2010, Calendar.MAY, 13)).isChanged ());
    assertEquals (PDTFactory.createDateTime (2010, DateTimeConstants.MAY, 13), aDTD.getValue ());
    // Set as Long
    assertTrue (aDTD.setValue (Long.valueOf (1270936800000L)).isChanged ());
    assertEquals (PDTFactory.createDateTime (2010, DateTimeConstants.APRIL, 11), aDTD.getValue ());
    // Set as String
    assertTrue (aDTD.setValue ("2010-04-09").isChanged ());
    assertEquals (PDTFactory.createDateTime (2010, DateTimeConstants.APRIL, 9), aDTD.getValue ());

    // Serialization
    assertEquals ("1270764000000", aDTD.getAsSerializationText ());
    assertTrue (aDTD.setAsSerializationText ("1270764000000").isSuccess ());
    // Cut time zone so that test works in all time zones :)
    assertEquals ("2010-04-09T00:00:00.000", _cutTimeZone (aDTD.getValue ().toString ()));
    ...
    PhlocTestUtils.testDefaultImplementationWithEqualContentObject (aDTD, new DynamicValueDateTime ());
    PhlocTestUtils.testDefaultImplementationWithDifferentContentObject (aDTD,
                                                                        new DynamicValueDateTime (PDTFactory.getCurrentDateTime ()));
  }
}
