...
public final class PropertyDescriptorCache
{
  ...
  static final class PDState
  {
    ...
    PDState (@Nonnull final Class <?> aClass,
             @Nonnull @Nonempty final String sField,
             @Nullable final String sReadMethodName,
             @Nullable final String sWriteMethodName)
    {
      if (aClass == null)
        throw new NullPointerException ("class");
      if (StringHelper.hasNoText (sField))
    ...
    public PropertyDescriptor createPropertyDescriptor ()
    {
      ...
      try
      {
        if (m_sReadMethodName == null && m_sWriteMethodName == null)
          aPD = new PropertyDescriptor (m_sField, m_aClass);
        else
          aPD = new PropertyDescriptor (m_sField, m_aClass, m_sReadMethodName, m_sWriteMethodName);
      }
      ...
      if (aReadMethod != null && !ClassHelper.isPublicClass (aReadMethod.getDeclaringClass ()))
        AccessControllerHelper.run (new PrivilegedActionAccessibleObjectSetAccessible (aReadMethod));

      final Method aWriteMethod = aPD.getWriteMethod ();
      if (aWriteMethod != null && !ClassHelper.isPublicClass (aWriteMethod.getDeclaringClass ()))
        AccessControllerHelper.run (new PrivilegedActionAccessibleObjectSetAccessible (aWriteMethod));
    ...
    public boolean equals (final Object o)
    {
      if (o == this)
        return true;
      if (!(o instanceof PDState))
        return false;
      final PDState rhs = (PDState) o;
      return m_aClass.equals (rhs.m_aClass) &&
             m_sField.equals (rhs.m_sField) &&
             EqualsUtils.equals (m_sReadMethodName, rhs.m_sReadMethodName) &&
             EqualsUtils.equals (m_sWriteMethodName, rhs.m_sWriteMethodName);
    }

    @Override
    public int hashCode ()
    {
      return new HashCodeGenerator (this).append (m_aClass)
                                         .append (m_sField)
                                         .append (m_sReadMethodName)
                                         .append (m_sWriteMethodName)
                                         .getHashCode ();
    }
    ...
    public String toString ()
    {
      return new ToStringGenerator (this).append ("class", m_aClass)
                                         .append ("field", m_sField)
                                         .appendIfNotNull ("readMethodName", m_sReadMethodName)
                                         .appendIfNotNull ("writeMethodName", m_sWriteMethodName)
                                         .toString ();
    }
  }

  // Use a WeakHashMap because PDState has a hard reference to a Class<?> object
  ...
  @Nonnull
  public static PropertyDescriptor getPropertyDescriptor (@Nonnull final Class <?> aClass,
                                                          @Nonnull @Nonempty final String sField)
  {
