...
public abstract class RandomAccessLongList extends AbstractLongCollection implements LongList
{
  ...
  private static class ComodChecker
  {
    ...
    ComodChecker (final RandomAccessLongList source)
    {
      _source = source;
      resyncModCount ();
    }
  ...
  protected static class RandomAccessLongListIterator extends ComodChecker implements LongListIterator
  {
    ...
    public long next ()
    {
      ...
      final long val = getList ().get (_nextIndex);
    ...
    public long previous ()
    {
      ...
      if (!hasPrevious ())
        throw new NoSuchElementException ();
      final long val = getList ().get (_nextIndex - 1);
  ...
  protected static class RandomAccessLongSubList extends RandomAccessLongList
  {
    ...
    RandomAccessLongSubList (final RandomAccessLongList list, final int fromIndex, final int toIndex)
    {
    ...
    public long removeElementAt (final int index)
    {
      checkRange (index);
      _comod.assertNotComodified ();
      final long val = _list.removeElementAt (toUnderlyingIndex (index));
      _limit--;
      _comod.resyncModCount ();
      incrModCount ();
      return val;
    ...
    public long set (final int index, final long element)
    {
      checkRange (index);
      _comod.assertNotComodified ();
      final long val = _list.set (toUnderlyingIndex (index), element);
      incrModCount ();
      _comod.resyncModCount ();
      return val;
    }
