...
public interface ShortList extends ShortCollection
{
   ...
   */
  void add (int nIndex, short aElement);
   ...
   * returned by the given collection's {@link ShortCollection#iterator iterator}.
   *
   * @param index
   *        the index at which to insert the first element from the specified
   *        collection
   * @param aCollection
   *        the {@link ShortCollection ByteCollection} of elements to add
   * @return <code>true</code> if I changed as a result of this call
   * @throws UnsupportedOperationException
   *         when this operation is not supported
   * @throws IndexOutOfBoundsException
   *         if the specified index is out of range
   */
  boolean addAll (int index, @Nonnull ShortCollection aCollection);

  /**
   * Returns <code>true</code> if <i>that</i> is an <code>ShortList</code>
   * that contains the same elements in the same order as me. In other words,
   * returns <code>true</code> if <i>that</i> is a <code>ShortList</code> that
   * has the same {@link #size() size} as me, and for which the elements
   * returned by its {@link ShortList#iterator iterator} are equal (
   ...
   * the <code>ShortList</code> interface.)
   *
   * @param that
   *        the object to compare to me
   * @return <code>true</code> if <i>that</i> is an <code>ShortList</code>
  ...
  /**
   * The hash code of an {@link ShortList} is defined to be the result of the
   ...
   */
  @Nonnull
  ShortListIterator listIterator (int nIndex);
   ...
   *         if the specified index is out of range
   */
  short removeElementAt (int nIndex);
   ...
   *         if the specified index is out of range
   */
  short set (int nIndex, short aElement);
   ...
   * <code>ShortList</code> is backed by me, so that any changes in the
   ...
   */
  @Nonnull
  ShortList subList (int nFromIndex, int nToIndex);
}
