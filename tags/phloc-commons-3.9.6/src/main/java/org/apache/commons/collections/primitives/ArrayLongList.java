...
public class ArrayLongList extends RandomAccessLongList implements Serializable
{
  private transient long [] m_aData;
  private int m_nSize;
   ...
   */
  public ArrayLongList (@Nonnull final long [] array)
  {
    this (array.length);
  ...
  public long removeElementAt (final int index)
  {
    _checkRange (index);
    incrModCount ();
    final long oldval = m_aData[index];
  ...
  @Override
  public boolean addAll (@Nonnull final LongCollection collection)
  {
  ...
  public void ensureCapacity (final int nMinCap)
  {
    ...
    if (nMinCap > m_aData.length)
    {
      ...
      final long [] olddata = m_aData;
      m_aData = new long [newcap < nMinCap ? nMinCap : newcap];
  ...
  private void writeObject (@Nonnull final ObjectOutputStream out) throws IOException
  {
    ...
    for (int i = 0; i < m_nSize; i++)
      out.writeLong (m_aData[i]);
  }

  private void readObject (@Nonnull final ObjectInputStream in) throws IOException, ClassNotFoundException
  {
    in.defaultReadObject ();
    m_aData = new long [in.readInt ()];
    for (int i = 0; i < m_nSize; i++)
      m_aData[i] = in.readLong ();
  }
