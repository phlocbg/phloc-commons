...
{
  private transient double [] m_aData;
  private int m_nSize;
   ...
   */
  public ArrayDoubleList (final int nInitialCapacity)
  {
    if (nInitialCapacity < 0)
      throw new IllegalArgumentException ("capacity " + nInitialCapacity);
    m_aData = new double [nInitialCapacity];
    m_nSize = 0;
  }
   ...
   *
   * @see ArrayDoubleList#addAll(DoubleCollection)
   * @param that
   *        the non-<code>null</code> collection of <code>double</code>s to add
   * @throws NullPointerException
   *         if <i>that</i> is <code>null</code>
   */
  public ArrayDoubleList (@Nonnull final DoubleCollection that)
  {
    this (that.size ());
    addAll (that);
  }
   ...
   */
  public ArrayDoubleList (@Nonnull final double [] array)
  {
    this (array.length);
  ...
  public double removeElementAt (final int index)
  {
    _checkRange (index);
    incrModCount ();
    final double oldval = m_aData[index];
  ...
  @Override
  public boolean addAll (@Nonnull final DoubleCollection collection)
  {
    return addAll (size (), collection);
  }

  @Override
  public boolean addAll (final int nIndex, @Nonnull final DoubleCollection collection)
  {
  ...
  public void ensureCapacity (final int nMinCap)
  {
    ...
    if (nMinCap > m_aData.length)
    {
      ...
      final double [] olddata = m_aData;
      m_aData = new double [newcap < nMinCap ? nMinCap : newcap];
  ...
  private void writeObject (@Nonnull final ObjectOutputStream out) throws IOException
  {
    ...
    for (int i = 0; i < m_nSize; i++)
      out.writeDouble (m_aData[i]);
  }

  private void readObject (@Nonnull final ObjectInputStream in) throws IOException, ClassNotFoundException
  {
    in.defaultReadObject ();
    m_aData = new double [in.readInt ()];
    for (int i = 0; i < m_nSize; i++)
      m_aData[i] = in.readDouble ();
  }
