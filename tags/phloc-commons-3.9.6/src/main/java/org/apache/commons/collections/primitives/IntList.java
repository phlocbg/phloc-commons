...
public interface IntList extends IntCollection
{
   ...
   */
  void add (int nIndex, int aElement);
   ...
   *        collection
   * @param aCollection
   *        the {@link IntCollection ByteCollection} of elements to add
   * @return <code>true</code> if I changed as a result of this call
   * @throws UnsupportedOperationException
   *         when this operation is not supported
   * @throws IndexOutOfBoundsException
   *         if the specified index is out of range
   */
  boolean addAll (int index, @Nonnull IntCollection aCollection);

  /**
   * Returns <code>true</code> if <i>that</i> is an <code>IntList</code>
   * that contains the same elements in the same order as me. In other words,
   * returns <code>true</code> if <i>that</i> is a <code>IntList</code> that
   * has the same {@link #size() size} as me, and for which the elements
   * returned by its {@link IntList#iterator iterator} are equal (
   ...
   * the <code>IntList</code> interface.)
   *
   * @param that
   *        the object to compare to me
   * @return <code>true</code> if <i>that</i> is an <code>IntList</code>
  ...
  /**
   * The hash code of an {@link IntList} is defined to be the result of the
   ...
   *         <code>-1</code> if no such matching element can be found
   */
  int lastIndexOf (int aElement);

  /**
   * @return a {@link IntListIterator bidirectional iterator} over all my
   ...
   */
  @Nonnull
  IntListIterator listIterator (int nIndex);
   ...
   *         if the specified index is out of range
   */
  int removeElementAt (int nIndex);
   ...
   *         if the specified index is out of range
   */
  int set (int nIndex, int aElement);
   ...
   * <code>IntList</code> is backed by me, so that any changes in the
   ...
   */
  @Nonnull
  IntList subList (int nFromIndex, int nToIndex);
}
