...
public abstract class RandomAccessByteList extends AbstractByteCollection implements ByteList
{
  ...
  {
    private RandomAccessByteList m_aSource;
    private int m_nExpectedModCount = -1;

    ComodChecker (@Nonnull final RandomAccessByteList source)
    {
      m_aSource = source;
      resyncModCount ();
    }
  ...
  protected static class RandomAccessByteListIterator extends ComodChecker implements ByteListIterator
  {
    ...
    public byte next ()
    {
      ...
      final byte val = getList ().get (m_nNextIndex);
    ...
    public byte previous ()
    {
      ...
      if (!hasPrevious ())
        throw new NoSuchElementException ();
      final byte val = getList ().get (m_nNextIndex - 1);
  ...
  protected static class RandomAccessByteSubList extends RandomAccessByteList
  {
    ...
    private RandomAccessByteList m_aList;
    private ComodChecker m_aComod;

    RandomAccessByteSubList (@Nonnull final RandomAccessByteList list, @Nonnegative final int fromIndex, @Nonnegative  final int toIndex)
    {
    ...
    public byte removeElementAt (final int index)
    {
      checkRange (index);
      m_aComod.assertNotComodified ();
      final byte val = m_aList.removeElementAt (toUnderlyingIndex (index));
      m_nLimit--;
      m_aComod.resyncModCount ();
      incrModCount ();
      return val;
    ...
    public byte set (final int index, final byte element)
    {
      checkRange (index);
      m_aComod.assertNotComodified ();
      final byte val = m_aList.set (toUnderlyingIndex (index), element);
      incrModCount ();
      m_aComod.resyncModCount ();
      return val;
    }
