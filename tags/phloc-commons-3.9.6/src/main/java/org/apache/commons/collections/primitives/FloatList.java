...
public interface FloatList extends FloatCollection
{
   ...
   */
  void add (int nIndex, float aElement);
   ...
   * returned by the given collection's {@link FloatCollection#iterator iterator}.
   *
   * @param index
   *        the index at which to insert the first element from the specified
   *        collection
   * @param aCollection
   *        the {@link FloatCollection ByteCollection} of elements to add
   * @return <code>true</code> if I changed as a result of this call
   * @throws UnsupportedOperationException
   *         when this operation is not supported
   * @throws IndexOutOfBoundsException
   *         if the specified index is out of range
   */
  boolean addAll (int index, @Nonnull FloatCollection aCollection);

  /**
   * Returns <code>true</code> if <i>that</i> is an <code>FloatList</code>
   * that contains the same elements in the same order as me. In other words,
   * returns <code>true</code> if <i>that</i> is a <code>FloatList</code> that
   * has the same {@link #size() size} as me, and for which the elements
   * returned by its {@link FloatList#iterator iterator} are equal (
   ...
   * the <code>FloatList</code> interface.)
   *
   * @param that
   *        the object to compare to me
   * @return <code>true</code> if <i>that</i> is an <code>FloatList</code>
  ...
  /**
   * The hash code of an {@link FloatList} is defined to be the result of the
   ...
   *         <code>-1</code> if no such matching element can be found
   */
  int lastIndexOf (float aElement);

  /**
   * @return a {@link FloatListIterator bidirectional iterator} over all my
   ...
   */
  @Nonnull
  FloatListIterator listIterator (int nIndex);
   ...
   *         if the specified index is out of range
   */
  float removeElementAt (int nIndex);
   ...
   *         if the specified index is out of range
   */
  float set (int nIndex, float aElement);
   ...
   * <code>FloatList</code> is backed by me, so that any changes in the
   ...
   */
  @Nonnull
  FloatList subList (int nFromIndex, int nToIndex);
}
