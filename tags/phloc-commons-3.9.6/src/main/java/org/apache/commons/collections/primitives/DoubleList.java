...
public interface DoubleList extends DoubleCollection
{
   ...
   */
  void add (int nIndex, double aElement);
   ...
   * returned by the given collection's {@link DoubleCollection#iterator iterator}.
   *
   * @param index
   *        the index at which to insert the first element from the specified
   *        collection
   * @param aCollection
   *        the {@link DoubleCollection ByteCollection} of elements to add
   * @return <code>true</code> if I changed as a result of this call
   * @throws UnsupportedOperationException
   *         when this operation is not supported
   * @throws IndexOutOfBoundsException
   *         if the specified index is out of range
   */
  boolean addAll (int index, @Nonnull DoubleCollection aCollection);

  /**
   * Returns <code>true</code> if <i>that</i> is an <code>DoubleList</code>
   * that contains the same elements in the same order as me. In other words,
   * returns <code>true</code> if <i>that</i> is a <code>DoubleList</code> that
   ...
   * @return <code>true</code> if <i>that</i> is an <code>DoubleList</code>
  ...
  /**
   * The hash code of an {@link DoubleList} is defined to be the result of the
   ...
   *         <code>-1</code> if no such matching element can be found
   */
  int lastIndexOf (double aElement);

  /**
   * @return a {@link DoubleListIterator bidirectional iterator} over all my
   ...
   */
  @Nonnull
  DoubleListIterator listIterator (int nIndex);
   ...
   *         if the specified index is out of range
   */
  double removeElementAt (int nIndex);
   ...
   *         if the specified index is out of range
   */
  double set (int nIndex, double aElement);
   ...
   */
  @Nonnull
  DoubleList subList (int nFromIndex, int nToIndex);
}
