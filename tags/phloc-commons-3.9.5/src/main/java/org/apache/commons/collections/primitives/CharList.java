...
public interface CharList extends CharCollection
{
   ...
   */
  void add (int nIndex, char aElement);
   ...
   *        collection
   * @param aCollection
   *        the {@link CharCollection ByteCollection} of elements to add
   * @return <code>true</code> if I changed as a result of this call
   * @throws UnsupportedOperationException
   *         when this operation is not supported
   * @throws IndexOutOfBoundsException
   *         if the specified index is out of range
   */
  boolean addAll (int index, @Nonnull CharCollection aCollection);

  /**
   * Returns <code>true</code> if <i>that</i> is an <code>CharList</code>
   * that contains the same elements in the same order as me. In other words,
   * returns <code>true</code> if <i>that</i> is a <code>CharList</code> that
   * has the same {@link #size() size} as me, and for which the elements
   * returned by its {@link CharList#iterator iterator} are equal (
   ...
   * the <code>CharList</code> interface.)
   *
   * @param that
   *        the object to compare to me
   * @return <code>true</code> if <i>that</i> is an <code>CharList</code>
  ...
  /**
   * The hash code of an {@link CharList} is defined to be the result of the
   ...
   */
  @Nonnull
  CharListIterator listIterator (int nIndex);
   ...
   *         if the specified index is out of range
   */
  char removeElementAt (int nIndex);
   ...
   *         if the specified index is out of range
   */
  char set (int nIndex, char aElement);
   ...
   * <code>CharList</code> is backed by me, so that any changes in the
   ...
   */
  @Nonnull
  CharList subList (int nFromIndex, int nToIndex);
}
