...
public class ArrayCharList extends RandomAccessCharList implements Serializable
{
  private transient char [] m_aData;
  private int m_nSize;
   ...
   */
  public ArrayCharList (@Nonnull final char [] array)
  {
    this (array.length);
  ...
  public char removeElementAt (final int index)
  {
    _checkRange (index);
    incrModCount ();
    final char oldval = m_aData[index];
  ...
  @Override
  public boolean addAll (@Nonnull final CharCollection collection)
  {
    return addAll (size (), collection);
  }

  @Override
  public boolean addAll (final int nIndex, @Nonnull final CharCollection collection)
  {
  ...
  public void ensureCapacity (final int nMinCap)
  {
    ...
    if (nMinCap > m_aData.length)
    {
      ...
      final char [] olddata = m_aData;
      m_aData = new char [newcap < nMinCap ? nMinCap : newcap];
  ...
  private void writeObject (@Nonnull final ObjectOutputStream out) throws IOException
  {
    ...
    for (int i = 0; i < m_nSize; i++)
      out.writeChar (m_aData[i]);
  }

  private void readObject (@Nonnull final ObjectInputStream in) throws IOException, ClassNotFoundException
  {
    in.defaultReadObject ();
    m_aData = new char [in.readInt ()];
    for (int i = 0; i < m_nSize; i++)
      m_aData[i] = in.readChar ();
  }
