 ...
 */
@Immutable
public final class CombinationGeneratorFlexibleOptimized <DATATYPE>
{
   ...
   */
  public CombinationGeneratorFlexibleOptimized (@Nonnegative final int nSlotCount, final boolean bAllowEmpty)
  {
    if (nSlotCount < 0)
  ...
  @Nonnull
  @ReturnsMutableCopy
  public Set <CombinationResultList <DATATYPE>> getCombinations (@Nonnull final List <DATATYPE> aElements)
  {
    if (aElements == null)
    ...
    final Set <CombinationResultList <DATATYPE>> aAllResults = new HashSet <CombinationResultList <DATATYPE>> ();
    for (int nSlotCount = m_bAllowEmpty ? 0 : 1; nSlotCount <= m_nSlotCount; nSlotCount++)
    {
      if (aElements.isEmpty ())
      {
        aAllResults.add (CombinationResultList.<DATATYPE> getEmpty ());
      }
      else
      {
        // Add all permutations for the current slot count
        CombinationGeneratorOptimized.addAllPermutations (aElements, nSlotCount, aAllResults);
        if (false)
          System.out.println ("Having " + aAllResults.size () + " results");
      }
    }
    return aAllResults;
  }
  ...
  @Nonnull
  @ReturnsMutableCopy
  public static <DATATYPE> Set <CombinationResultList <DATATYPE>> getCombinations (@Nonnull final List <DATATYPE> aElements,
                                                                                   final boolean bAllowEmpty)
  {
    return new CombinationGeneratorFlexibleOptimized <DATATYPE> (aElements.size (), bAllowEmpty).getCombinations (aElements);
  }
}
