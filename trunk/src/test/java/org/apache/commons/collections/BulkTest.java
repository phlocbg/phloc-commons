 ...
 * <p>
 * A <I>simple test method</I> is the type of test traditionally supplied by by
 * {@link TestCase}. To define a simple test, create a public no-argument method
 * whose name starts with "test". You can specify the the name of simple test in
 * the constructor of <code>BulkTest</code>; a subsequent call to
 * {@link TestCase#run} will run that simple test.
 * <p>
 * A <I>bulk test method</I>, on the other hand, returns a new instance of
 * <code>BulkTest</code>, which can itself define new simple and bulk test
 * methods. By using the {@link #makeSuite} method, you can automatically create
 * a hierarchal suite of tests and child bulk tests.
 * <p>
 ...
 * and no bulk test methods; <code>TestHashMap</code> defines one simple test
 * method and two bulk test methods. When
 * <code>makeSuite(TestHashMap.class).run</code> is executed, <I>five</I> simple
 * test methods will be run, in this order:
 * <P>
 * <Ol>
 ...
 * </Ol>
 * In the graphical junit test runners, the tests would be displayed in the
 * following tree:
 * <P>
 * <UL>
 * <LI>TestHashMap</LI>
 ...
 * </UL>
 * </UL>
 * </UL> A subclass can override a superclass's bulk test by returning
 * <code>null</code> from the bulk test method. If you only want to override
 * specific simple tests within a bulk test, use the {@link #ignoredTests}
 * method.
 * <P>
 * Note that if you want to use the bulk test methods, you <I>must</I> define
 * your <code>suite()</code> method to use {@link #makeSuite}. The ordinary
 * {@link TestSuite} constructor doesn't know how to interpret bulk test
 * methods.
 * 
 * @author Paul Jack
...
public class BulkTest extends TestCase implements Cloneable
{
  ...
  /**
   * The full name of this bulk test instance. This is the full name that is
   * compared to {@link #ignoredTests} to see if this test should be ignored.
   * It's also displayed in the text runner to ease debugging.
   */
  String verboseName;

  /**
   * Constructs a new <code>BulkTest</code> instance that will run the specified
   * simple test.
   * 
   * @param name
   *        the name of the simple test method to run
   */
  ...
  /**
   * Creates a clone of this <code>BulkTest</code>.
   * <P>
   * 
   ...
   * <P>
   * If a test that's defined by this <code>BulkTest</code> or by one of its
   * bulk test methods has a name that's in the returned array, then that simple
   * test will not be executed.
   * <P>
   * A test's name is formed by taking the class name of the root
   * <code>BulkTest</code>, eliminating the package name, then appending the
   * names of any bulk test methods that were invoked to get to the simple test,
   * and then appending the simple test method name. The method names are
   ...
   * described above. If the sample <code>TestHashMap</code> class included this
   * method:
   * 
   * <pre>
   ...
   * </pre>
   * 
   * then the entry set's clear method wouldn't be tested, but the key set's
   * clear method would.
   * 
   * @return an array of the names of tests to ignore, or null if no tests
   *         should be ignored
   */
  public String [] ignoredTests ()
  {
  ...
  /**
   * Returns a {@link TestSuite} for testing all of the simple tests <I>and</I>
   * all the bulk tests defined by the given class.
   * <P>
   * The class is examined for simple and bulk test methods; any child bulk
   * tests are also examined recursively; and the results are stored in a
   * hierarchal {@link TestSuite}.
   * <P>
   * The given class must be a subclass of <code>BulkTest</code> and must not be
   * abstract.
   * <P>
   * 
   * @param c
   *        the class to examine for simple and bulk tests
   * @return a {@link TestSuite} containing all the simple and bulk tests
   *         defined by that class
   */
  public static TestSuite makeSuite (final Class <?> c)
  {
...
class BulkTestSuiteMaker
{
  ...
  /**
   * The prefix for simple test methods. Used to check if a test is in the
   * ignored list.
   */
   ...
   * Constructor.
   * 
   * @param m_aStartingClass
   *        the starting class
   */
  public BulkTestSuiteMaker (final Class <?> m_aStartingClass)
  {
    this.startingClass = m_aStartingClass;
  }
  ...
  /**
   * Appends all the simple tests and bulk tests defined by the given instance's
   * class to the current TestSuite.
   * 
   * @param bulk
   *        An instance of the class that defines simple and bulk tests for us
   *        to append
   */
   ...
   * 
   * @param bulk
   *        The instance of the class that defined the method (I know it's
   *        weird. But the point is, we can clone the instance and not have to
   *        worry about constructors.)
   * @param m
  ...
  /**
   * Adds a whole new suite of tests that are defined by the result of the given
   * bulk test method. In other words, the given bulk test method is invoked,
   * and the resulting BulkTest instance is examined for yet more simple and
   * bulk tests.
   * 
   * @param bulk
   *        The instance of the class that defined the method
   * @param m
   *        The bulk test method
   */
  ...
  private static Constructor <?> getTestCaseConstructor (final Class <?> c)
  {
    try
    {
  ...
  private static BulkTest makeTestCase (final Class <?> c, final Method m)
  {
    final Constructor <?> con = getTestCaseConstructor (c);
    try
    {
  ...
  private static BulkTest makeFirstTestCase (final Class <?> c)
  {
    final Method [] all = c.getMethods ();
    for (final Method element : all)
    {
