...
public final class XMLHelper
{
   ...
   * @return <code>true</code> if the char is invalid
   */
  public static boolean isInvalidNameChar (final int c)
  {
    return XML_INVALID_NAME_CHAR.get (c);
  }

  /**
   * Check if the passed character is invalid for an attribute value, a text
   * node or a CDATA node.
   * 
   * @param c
   *        char to check
   * @return <code>true</code> if the char is invalid
   */
  public static boolean isInvalidValueChar (final int c)
  {
    return XML_INVALID_VALUE_CHAR.get (c);
  }
}
